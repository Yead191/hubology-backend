...
const createReportToDB = async (user: JwtPayload, payload: IReport) => {
  ...
  try {
    ...
    if (
      updatedPost &&
      updatedPost.reportCount >= REPORT_THRESHOLD &&
...
const getAllReportsFromDb = async (query: Record<string, any>) => {
  const qb = new QueryBuilder(
    Report.find()
      .populate('reporter', 'email name image')
      .populate({
        path: 'post',
        select:
          'author category content image totalLikes totalComments reportCount status',
        populate: {
          path: 'author',
...
const getReportsByPost = async (id: string) => {
  const reports = await Report.find({ post: id }).populate(
    'reporter',
    'email name image',
  );
  if (!reports) {
