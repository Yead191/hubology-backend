...
const getSinglePostFromDB = async (id: string, user: JwtPayload) => {
  ...
  const isLiked = await Like.isLikeByMe(user.id as any, id as any);
...
const updatePostToDB = async (
  ...
  if (isPostExist.status === 'removed') {
    throw new ApiError(
      ...
      'This Post is removed by admin and cannot be updated!',
    );
  }
...
const deletePostFromDB = async (user: JwtPayload, id: string) => {
  ...
  if (isPostExist.status === 'removed') {
    throw new ApiError(
      StatusCodes.BAD_REQUEST,
      'This Post is already removed by admin and will track on admin panel!',
    );
  }
